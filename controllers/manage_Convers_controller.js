...
// handle last message of the conversation seen by the target
export const handleLastMessageConversationSeen = async (req, res) => {
  try {
    // obtain the ID from the request params and update the target seen or read
    ...
    // return the response to the frontend
    res.status(200).send("target has seen the last conversation message");
  } catch (error) {
    // debug
    console.log(error);
    // send error to the frontend
...
// handle deletion of the message
export const handleDeleteMessageConversation = async (req, res) => {
  try {
    // obtain the data from the body request
